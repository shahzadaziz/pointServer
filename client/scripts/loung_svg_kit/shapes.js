...
function lounge_shapes(svg,domsvg){


  function remove_Shape(oGWrap){
  ...
  function lounge_shapes_add(jsonData){
  ...
  function lounge_shapes_addbtn(){
   
    var oGlist = this.domsvg.getElementsByTagName('g');
    oLastElm = oGlist.item(oGlist.length -2);
    oLastElm = getShapeElement(oLastElm);

    oAddbtn = this.svg.append('g');
    oAddbtn.append('image').attr('x', 300).attr('y','20').attr('href', '/images/head/png');
    
    
    
//    select.forEach(function(oG){
//      log(select);
//
//    });
  ...
  function lounge_shapes_element(s_jsondata,x,y){
    ...
    oGDelete.append("text").text("x").attr('style','font-family: Dotum; font-size: 14px;').attr("x", x+34).attr("y", y+107).attr("fill",this.baseText).attr('pointer-events','none');
    ...
    oGEdit.append("text").text("e").attr('style','font-family: Dotum; font-size: 14px;').attr("x", x+54).attr("y", y+107).attr("fill",this.baseText).attr('pointer-events','none');

    var oGInfo = oGToolbar.append("g")
    oGInfo.append("text").text("open >").attr('style','font-family: Dotum; font-size: 12px;').attr("x", x+74).attr("y", y+107).attr("fill","#000").attr('pointer-events','none').attr('pointer-events','none');
  ...
  //Theme colors
  this.baseText = '#fff';
  this.baseTextStyle = 'font-family: Dotum; font-size: 25px';
  ...
  this.element = lounge_shapes_element;
  this.drawadd = lounge_shapes_addbtn;
  this.addShape = lounge_shapes_add;
  this.removeShape = remove_Shape;
  this.tile_width = 120;
